...
- [x] 里程碑 B：25 张情况卡、三级活动提示、8 周冷却、稀有上限和确定性抽取已实现。其中 20 张为初始牌，5 张用于区分新增职业经历。
- [~] 里程碑 C：12 维证据、每周证据上限、风险时间窗、30 个结局评分和近邻路线建议已实现；第 20 周方向仍需补“已有证据 / 尚待观察”的成对文案。
- [x] 里程碑 D：30 个结局、纯 ASCII 字符画、独立图鉴提示、活动签名和关键情况资格已实现。
- [~] 里程碑 E：19 项规则测试、28 条职业签名可满足性测试、10,000 局多策略分布模拟、生产构建和本地浏览器冒烟通过；GitHub Pages 发布验证待完成。
...
| 14 | `data_scientist` | 数据研究员 | 研究能力、产品判断 | 数据分析事件、实验与业务问题结合 |
| 15 | `applied_scientist` | 应用研究员 | 研究能力、责任感 | 研究证据高且能把实验结果交付给团队 |
...
- HumanDex 保留现有 Trait 和结局发现记录；旧 `developer_relations` 自动映射为 `technical_community`。
